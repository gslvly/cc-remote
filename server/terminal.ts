...
/** pid → 父进程和启动时间。pid 活着就一直有效，查过的不再查 */
const procs = new Map<number, { ppid: number; start: string }>()

/** 一次 ps 查多个进程。lstart 在 LC_ALL=C TZ=UTC 下的格式与 procStart 相同；有 pid 查不到时 ps 退出码非 0，但查到的照样输出 */
async function lookup(pids: number[]) {
  if (!pids.length) return
  try {
    const ps = Bun.spawn(['ps', '-o', 'pid=,ppid=,lstart=', '-p', pids.join(',')], {
    ...
    for (const line of (await new Response(ps.stdout).text()).split('\n')) {
      ...
      if (m) procs.set(Number(m[1]), { ppid: Number(m[2]), start: m[3]! })
    }
  } catch (e) {
...
/** sessionId → 持有它的终端进程。服务端自己起的 claude 子进程也会登记（entrypoint 为 sdk-ts），按父进程排除 */
export async function scanHolders(): Promise<Map<string, Holder>> {
  ...
  const live = records.filter(valid).filter((r) => KINDS.includes(r.kind ?? 'interactive') && !r.parkedJobId && alive(r.pid))

  const pids = new Set(live.map((r) => r.pid))
  for (const pid of procs.keys()) if (!pids.has(pid)) procs.delete(pid)
  await lookup(live.filter((r) => !procs.has(r.pid) || (r.procStart && procs.get(r.pid)!.start !== r.procStart)).map((r) => r.pid))

  const out = new Map<string, Holder>()
  for (const r of live) {
    const p = procs.get(r.pid)
    if (!p || p.ppid === process.pid) continue
    // 老版本没有 procStart，只能信 pid
    if (r.procStart && p.start !== r.procStart) continue
